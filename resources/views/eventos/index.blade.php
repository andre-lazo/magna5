...
<table class="table table-bordered table-striped mb-0 table-hover">
  ...
  <tbody>
    ...
    @include('eventos.model_delete')
    <tr>
      <th scope="row">{{$evento->id}}</th>
      <td>{{$evento->usuario}}</td>
      <td>{{$evento->hora}}</td>
      <td>{{$evento->start}}</td>
      <td class="text-center"><a href="{{route('eventos.edit',$evento->id)}}" class="btn btn-success"><i class="far fa-eye"></i>Pdf</a></td>
      <td class="text-center"><button type="button" class=" btn btn-outline-danger" data-toggle="modal" data-target="#modalEliminar-{{$evento->id}}"><i class="far fa-trash-alt"></i> Eliminar</button></td>
...
<!--MODAL-->
<form action="{{route('eventos.create')}}">
  @csrf
